...
- Event bus = in-process `asyncio.Queue` per subscriber, keyed by `session_id` (the frontend `clientId`). Delivery is per-session — no cross-client fan-out. No external broker. Events lost on process restart.
