...
**Agent** — the Pipecat pipeline's LLM component (Claude Sonnet 4.6). Receives finalized transcripts, emits tool calls against the service layer. Text-only feedback (ADR 0005) — no TTS; the UI reflects each action live as it executes.
